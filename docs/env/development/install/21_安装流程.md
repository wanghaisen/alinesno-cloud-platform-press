...
下载安装包，通过wget命令，如下：
```shell
wget http://data.linesno.com/alinesno-cloud-platform-install-boot/alinesno-cloud-platform-install-boot.jar
```

运行安装程序,这里的端口是`28080`，注意端口开放：
```shell
java -jar alinesno-cloud-platform-install-boot.jar
```
